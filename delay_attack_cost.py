...
import attacks.deepsloth_linf as DeepSloth
from msdnet_util.profiler import profile_sdn
...
def convert_num_early_exits_at_each_ic_to_cumulative_dis(ic_exits, total_samples):
    ...
    layer_cumul_dist[-1] = total_samples
    layer_cumul_dist = [val / total_samples for val in layer_cumul_dist]
    print("run_ours_attacks: ",layer_cumul_dist)
    return layer_cumul_dist
...
def test_and_save_threshold_results( \
    ...
    if adv:
        ...
        if 'vgg' in network or 'resnet' in network or 'mobile' in network:
            sdn_name = task + '_' + network + '_sdn_ic_only'
        else:
    ...
    if utils.file_exists(save_pickle):
        ...
        sdn_model, sdn_params = models.load_model(models_path, sdn_name, epoch=-1)
        sdn_model.to(device)
        c_i, _ = profile_sdn(sdn_model, sdn_model.input_size, device,vgg= 'vgg' in network)
        ...
        if manual_loader is not None:
            ...
            dataset = utils.load_dataset(task, doNormalization=sdn_params.get('doNormalization', False))
            one_batch_dataset = utils.load_dataset(task, batch_size=1, doNormalization=sdn_params.get('doNormalization', False))
        ...
        for threshold in confidence_thresholds:
            ...
            print(f"early exit counts {early_exit_counts}")
...
def compute_delay_metric_w_loader( \
    ...
    if utils.file_exists(save_pickle):
        ...
        if 'vgg' in network:
            c_i = profile_sdn(sdn_model, sdn_model.input_size, device, vgg=True)[0]
        else:
            c_i = profile_sdn(sdn_model, sdn_model.input_size, device, vgg=False)[0]
        print("c_i", c_i)
