...
class TinyImagenetSubset():
    def __init__(self, dataroot=None, batch_size=128, doNormalization=True):
        print('Loading TinyImageNet Subset...')
        self.batch_size = batch_size
        self.img_size = 64
        self.num_classes = 10  # Only loading first 10 classes
        ...
        # Load the datasets, but filter for the first 10 classes
        self.aug_trainset = self._get_subset(train_dir, self.augmented)
        self.aug_train_loader = torch.utils.data.DataLoader(self.aug_trainset, batch_size=batch_size, shuffle=True, num_workers=8)

        self.trainset = self._get_subset(train_dir, self.normalized)
        self.train_loader = torch.utils.data.DataLoader(self.trainset, batch_size=batch_size, shuffle=True)

        self.testset = self._get_subset(valid_dir, self.normalized)
        self.test_loader = torch.utils.data.DataLoader(self.testset, batch_size=batch_size, shuffle=False)

    def _get_subset(self, directory, transform):
        dataset = datasets.ImageFolder(directory, transform=transform)
        # Get indices of images belonging to the first `self.num_classes` classes
        class_indices = []
...
def load_train_loader(dataset, nbatch=1, normalize=False):
    ...
    elif 'tinyimagenetsub' == dataset:
        return TinyImagenetSubset(batch_size=nbatch, doNormalization=normalize).train_loader
    else:
...
def load_valid_loader(dataset, nbatch=1, normalize=False):
    if 'mnist' == dataset:
        ...
        return TinyImagenetSubset(batch_size=nbatch, doNormalization=normalize).test_loader
    else:
        assert False, ('Error: unsupported dataset - {}'.format(dataset))
    # done
