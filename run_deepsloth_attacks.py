...
def run_attack(args, use_cuda=False):
    ...
    elif 'deepsloth-class' == args.attacks:
        save_folder = os.path.join( \
            'samples', args.dataset, netname)
        if not os.path.exists(save_folder): os.makedirs(save_folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--attacks', type=str, default='PGD',
                        help='the attack that this script will use (PGD, PGD-avg, PGD-max, UAP, deepsloth, deepsloth-class)')
