...
class MSNet(nn.Module):
    def __init__(self, args):
        ...
        for i in range(self.nBlocks):
            ...
            if args.data.startswith('cifar100'):
                ...
                self.input_size = 64
                self.classifier.append(
                    self._build_classifier_cifar(nIn * args.grFactor[outScales - 1], 200))
            else:
    ...
    def _build_classifier_tinyimagenet(self, nIn, num_classes):
        if nIn >= 256:
            interChannels1, interChannels2 = 128, 128
        elif nIn >= 128:
            interChannels1, interChannels2 = 64, 64
        ...
        conv = nn.Sequential(
            BasicConv(nIn, interChannels1, kernel_size=3, stride=2, padding=1),
            BasicConv(interChannels1, interChannels2, kernel_size=3, stride=2, padding=1),
            BasicConv(interChannels2, interChannels2, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(interChannels2),
            nn.ReLU(inplace=True)
        )
        return ClassifierModule(conv, interChannels2, num_classes)
    ...
    def early_exit(self, x):
        ...
        if self.output_to_return_when_ICs_are_delayed == 'most_confident_output':
            ...
            raise RuntimeError(
                'Invalid value for "output_to_return_when_ICs_are_delayed": it should be "network_output" or "most_confident_output"')
