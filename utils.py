...
import models
from datasets import CIFAR10, CIFAR100, TinyImagenet, SVHN, TinyImagenetSubset
...
def load_dataset(dataset, batch_size=128, doNormalization=True):
    ...
    elif dataset == 'tinyimagenetsub':
        return load_tinyimagenet_subset(batch_size, doNormalization)
    else:
...
def load_tinyimagenet_subset(batch_size, doNormalization=True):
    return TinyImagenetSubset(batch_size=batch_size, doNormalization=doNormalization)
